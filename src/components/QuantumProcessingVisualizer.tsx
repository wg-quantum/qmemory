...
export default function QuantumProcessingVisualizer({ 
  ...
  useEffect(() => {
    ...
    // Generate random memory fragments
    const fragments = Array.from({ length: 6 }, () => 
      memoryFragmentList[Math.floor(Math.random() * memoryFragmentList.length)]
    )
    ...
    const processStage = () => {
      if (stageIndex < stages.length) {
        ...
        setTimeout(() => {
          ...
          // Show quantum observation during measurement and collapse stages
          if (stages[stageIndex].id === 'measurement' && mainLocation && secondaryLocations) {
